...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const CHAIN_COLORS = [
    '#E4256D', '#2495C1', '#F5A623', '#7B4B94', '#3CA474',
    '#D63384', '#0DCAF0', '#FFC107', '#6F42C1', '#20C997',
  ];
  const BOND_ALPHA = 0.45;
  const SOLVENT_COLOR = 'rgba(154, 163, 171, 0.50)';
  const SOLVENT_COUNT = 120;
  const BEAD_RADIUS = 5;
  const SOLVENT_RADIUS = 1.8;

  // --- Physics parameters ---
  const DT = 0.35;
  const FRICTION = 0.99;
  const THERMAL_KICK = 0.22;
  let REPULSION_STRENGTH = 100;
  let REPULSION_CUTOFF = BEAD_RADIUS * 3.5;
  const BOND_LENGTH = 20;
  const CONSTRAINT_ITERS = 4;
  const ANGLE_STIFFNESS = 0.015;
  const SHEAR_RATE = 0.015;
  ...
  function makeLinearBonds(n) {
    const b = []; for (let i = 0; i < n - 1; i++) b.push([i, i + 1]); return b;
  }
  function makeCyclicBonds(n) {
    const b = makeLinearBonds(n); b.push([n - 1, 0]); return b;
  }

  // --- Chain constructors ---
  function createLinearChain(n, color, sx, sy) {
    const beads = []; let x = sx, y = sy;
    for (let i = 0; i < n; i++) {
      beads.push({ x: x + (Math.random() - 0.5) * 4, y: y + (Math.random() - 0.5) * 4, vx: 0, vy: 0 });
      const a = Math.random() * Math.PI * 2;
      x += Math.cos(a) * BOND_LENGTH; y += Math.sin(a) * BOND_LENGTH;
    }
    return { beads, bonds: makeLinearBonds(n), color };
  }

  function createCyclicChain(n, color, sx, sy) {
    const beads = []; const r = (n * BOND_LENGTH) / (2 * Math.PI);
    for (let i = 0; i < n; i++) {
  ...
  function createStarChain(nArms, armLen, color, sx, sy) {
    ...
    for (let arm = 0; arm < nArms; arm++) {
      ...
      for (let i = 0; i < armLen; i++) {
        x += Math.cos(angle) * BOND_LENGTH; y += Math.sin(angle) * BOND_LENGTH;
  ...
  function createBranchedChain(backboneLen, nBranches, branchLen, color, sx, sy) {
    const beads = []; const bonds = []; let x = sx, y = sy;
    for (let i = 0; i < backboneLen; i++) {
      beads.push({ x, y, vx: 0, vy: 0 }); x += BOND_LENGTH;
      if (i < backboneLen - 1) bonds.push([i, i + 1]);
    }
    const step = Math.max(2, Math.floor(backboneLen / (nBranches + 1)));
    for (let b = 0; b < nBranches; b++) {
      const ai = step * (b + 1); if (ai >= backboneLen) break;
      const main = beads[ai]; let bx = main.x, by = main.y + BOND_LENGTH;
      const base = beads.length; beads.push({ x: bx, y: by, vx: 0, vy: 0 });
      bonds.push([ai, base]);
      for (let i = 1; i < branchLen; i++) {
        by += BOND_LENGTH; beads.push({ x: bx, y: by, vx: 0, vy: 0 });
  ...
  const SEGMENT_CUTOFF = BEAD_RADIUS * 3.0;
  const SEGMENT_STRENGTH = 60;

  function initChains() {
    chains = [];
    // Staggered grid placement — each chain gets its own cell
    const cols = 4, rows = 3;
    const cellW = (width - 80) / cols, cellH = (height - 80) / rows;
    const specs = [
    ...
    for (let i = 0; i < specs.length; i++) {
      ...
      const cy = 50 + cellH * (row + 0.5);
      const chain = specs[i].fn(...specs[i].args, cx, cy);
      chains.push(chain);
    }
    // Silent warmup — 500 steps with no rendering, stronger repulsion
    ...
    for (const chain of chains) {
      for (const bead of chain.beads) { bead.vx = 0; bead.vy = 0; }
    }
  }
  ...
  function enforceBondConstraints(chain) {
    const beads = chain.beads, bonds = chain.bonds;
    for (let iter = 0; iter < CONSTRAINT_ITERS; iter++) {
      for (const [i, j] of bonds) {
        const a = beads[i], b = beads[j];
        let dx = wrapDelta(a.x - b.x, width), dy = wrapDelta(a.y - b.y, height);
        const r = Math.sqrt(dx * dx + dy * dy);
        if (r < 0.001) continue;
        const corr = (BOND_LENGTH - r) / r * 0.5;
        a.x += dx * corr; a.y += dy * corr;
        b.x -= dx * corr; b.y -= dy * corr;
        a.x = wrap(a.x, width); a.y = wrap(a.y, height);
        b.x = wrap(b.x, width); b.y = wrap(b.y, height);
      }
    }
  }
    ...
    for (const [mid, neighbors] of Object.entries(adj)) {
      ...
      for (let p = 0; p < neighbors.length; p++) {
        for (let q = p + 1; q < neighbors.length; q++) {
          const a = beads[neighbors[p]], c = beads[neighbors[q]];
          let dx1 = wrapDelta(a.x - b.x, width), dy1 = wrapDelta(a.y - b.y, height);
          let dx2 = wrapDelta(c.x - b.x, width), dy2 = wrapDelta(c.y - b.y, height);
          const r1 = Math.sqrt(dx1 * dx1 + dy1 * dy1), r2 = Math.sqrt(dx2 * dx2 + dy2 * dy2);
          if (r1 < 0.001 || r2 < 0.001) continue;
          dx1 /= r1; dy1 /= r1; dx2 /= r2; dy2 /= r2;
          const cs = dx1 * dx2 + dy1 * dy2;
          const f = ANGLE_STIFFNESS * (1 - cs);
          const p1x = dx2 - cs * dx1, p1y = dy2 - cs * dy1;
          const p2x = dx1 - cs * dx2, p2y = dy1 - cs * dy2;
          a.vx += p1x * f * DT; a.vy += p1y * f * DT;
          c.vx += p2x * f * DT; c.vy += p2y * f * DT;
          b.vx -= (p1x + p2x) * f * DT; b.vy -= (p1y + p2y) * f * DT;
        }
      }
    }
  }
  ...
  function segmentDist2(a, b, c, d) {
    let abx = wrapDelta(b.x - a.x, width), aby = wrapDelta(b.y - a.y, height);
    let cdx = wrapDelta(d.x - c.x, width), cdy = wrapDelta(d.y - c.y, height);
    let acx = wrapDelta(c.x - a.x, width), acy = wrapDelta(c.y - a.y, height);
    let t = (acx * cdx + acy * cdy) / (cdx * cdx + cdy * cdy + 0.0001);
    t = Math.max(0, Math.min(1, t));
    const nearX = c.x + cdx * t, nearY = c.y + cdy * t;
    let dax = wrapDelta(nearX - a.x, width), day = wrapDelta(nearY - a.y, height);
    let bestDist2 = dax * dax + day * day;
    let bcx = wrapDelta(c.x - b.x, width), bcy = wrapDelta(c.y - b.y, height);
    t = (bcx * cdx + bcy * cdy) / (cdx * cdx + cdy * cdy + 0.0001);
    t = Math.max(0, Math.min(1, t));
    const nearX2 = c.x + cdx * t, nearY2 = c.y + cdy * t;
    let dbx = wrapDelta(nearX2 - b.x, width), dby = wrapDelta(nearY2 - b.y, height);
    ...
    for (let ci = 0; ci < n; ci++) {
      ...
      for (let cj = ci + 1; cj < n; cj++) {
        ...
        for (const [ai, aj] of ca.bonds) {
          const a = ca.beads[ai], b = ca.beads[aj];
          for (const [bi, bj] of cb.bonds) {
            const c = cb.beads[bi], d = cb.beads[bj];
            const d2 = segmentDist2(a, b, c, d);
            if (d2 < SEGMENT_CUTOFF * SEGMENT_CUTOFF) {
              const force = SEGMENT_STRENGTH / (d2 + 0.001);
              const mabx = (a.x + b.x) / 2, maby = (a.y + b.y) / 2;
              const mcdx = (c.x + d.x) / 2, mcdy = (c.y + d.y) / 2;
              let dx = wrapDelta(mabx - mcdx, width);
              let dy = wrapDelta(maby - mcdy, height);
              const mag = Math.sqrt(dx * dx + dy * dy) + 0.001;
              dx /= mag; dy /= mag;
              const f2 = force * 2;
              a.vx += dx * f2 * DT; a.vy += dy * f2 * DT;
              b.vx += dx * f2 * DT; b.vy += dy * f2 * DT;
              c.vx -= dx * f2 * DT; c.vy -= dy * f2 * DT;
              d.vx -= dx * f2 * DT; d.vy -= dy * f2 * DT;
            }
          }
        }
      }
  ...
  // --- Core physics (reusable for warmup) ---
  function stepPhysics() {
    ...
    for (let i = 0; i < allBeads.length; i++) {
      for (let j = i + 1; j < allBeads.length; j++) {
        const ia = allBeads[i], ja = allBeads[j];
        if (ia.ci === ja.ci) {
          const c = chains[ia.ci];
          const bonded = c.bonds.some(([a, b]) =>
            (a === ia.bi && b === ja.bi) || (a === ja.bi && b === ia.bi));
          if (bonded) continue;
        }
        const a = ia.bead, b = ja.bead;
        let dx = wrapDelta(a.x - b.x, width), dy = wrapDelta(a.y - b.y, height);
    ...
    for (const s of solvent) {
      ...
      s.x = wrap(s.x, width); s.y = wrap(s.y, height);
    }
  }

  // --- Physics step (called each frame) ---
  function step() {
    stepPhysics();
  }
  ...
  function findNearestBead(mx, my) {
    ...
    chains.forEach((chain, ci) => chain.beads.forEach((bead, bi) => {
      let dx = wrapDelta(bead.x - mx, width), dy = wrapDelta(bead.y - my, height);
      const d = Math.sqrt(dx * dx + dy * dy);
      if (d < bestDist) { bestDist = d; best = { ci, bi }; }
    }));
    return best;
  }
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    if (dragBead) {
      ...
      b.x = mouseX; b.y = mouseY; b.vx = 0; b.vy = 0;
  ...
  canvas.addEventListener('mouseup', () => { dragBead = null; canvas.style.cursor = 'default'; });
  canvas.addEventListener('mouseleave', () => { dragBead = null; canvas.style.cursor = 'default'; });
  ...
  canvas.addEventListener('touchend', () => { dragBead = null; });

  // --- Rendering ---
  function drawBond(ax, ay, bx, by, color) {
    let dx = wrapDelta(bx - ax, width), dy = wrapDelta(by - ay, height);
    ctx.beginPath(); ctx.moveTo(ax, ay); ctx.lineTo(ax + dx, ay + dy);
    ctx.strokeStyle = color; ctx.lineWidth = 1.6;
    ctx.globalAlpha = BOND_ALPHA; ctx.stroke(); ctx.globalAlpha = 1;
  }

  function draw() {
    ctx.clearRect(0, 0, width, height);
    for (const s of solvent) {
      ctx.beginPath(); ctx.arc(s.x, s.y, SOLVENT_RADIUS, 0, Math.PI * 2);
      ctx.fillStyle = SOLVENT_COLOR; ctx.fill();
    }
    for (const chain of chains) {
      ...
      for (const bead of chain.beads) {
        ctx.beginPath(); ctx.arc(bead.x, bead.y, BEAD_RADIUS, 0, Math.PI * 2);
        ctx.fillStyle = chain.color; ctx.fill();
        ctx.beginPath(); ctx.arc(bead.x - 1.2, bead.y - 1.2, BEAD_RADIUS * 0.4, 0, Math.PI * 2);
  ...
  resize(); initChains(); initSolvent();
  window.addEventListener('resize', () => { resize(); initChains(); initSolvent(); });
  loop();
});
